...
def arima_prediction(data, len_test):
    data = data.transpose(0,3,1,2)
    train_data, test_data = data[:-len_test], data[-len_test:] # 从数据中分离出训练集和测试集
    ...
    for flow in [0]:
        for row in range(num_rows):
            for column in range(num_columns): # 对网格中的每个区域进行预测
                history_region = [x[flow][row][column] for x in train_data]
                history_region = np.array(history_region) # 转换为numpy数组
...
def main():
    ...
    output_window = config.get('output_window', 3)
    data = np.load(config.get('filename', 'cd_taxi.npy'))
    # data = np.squeeze(data,axis=-1)
    # trainx, trainy, testx, testy = preprocess_data(data, config)
    # y_pred = arima(testx)
    y_pred = arima_prediction(data, output_window)
    testy = data[-output_window:]
