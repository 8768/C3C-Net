...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.load_external = self.config.get('load_external', False) # 是否加载外部数据
        self.normal_external = self.config.get('normal_external', False) # 是否对外部数据进行归一化
        self.add_time_in_day = self.config.get('add_time_in_day', False) # 是否添加一天中的时刻
        self.add_day_in_week = self.config.get('add_day_in_week', False) # 是否添加一周中的星期几
        ...
        self.data_path = './data_process/output/' + self.dataset + '/'
    ...
    def _add_external_information_4d(self, df, ext_data=None):
        ...
        #     distance = np.tile(distance, [len(self.timesolts), 1, 1]).reshape(
        #         (len(self.timesolts), len_row, len_column, 1))
        ...
        #     P = minmax_norm(P)
        #     P = P.values
        #     data_p = np.zeros((len_row, len_column, 2 * P.shape[1]))
        #     for i in range(P.shape[0]):
        #         p_i = P[i, :]
        #         for j in range(P.shape[0]):
        #             p_j = P[j, :]
        #             data_p[i][j] = np.hstack((p_i, p_j))
        #     data_p = np.tile(data_p, [len(self.timesolts), 1, 1, 1])
    ...
    def _generate_data(self):
        ...
        for filename in data_files:
            ...
            print(np.sum(df >= 1))
            print(np.sum(df == 0))
            self._logger.info("total count: {}, empty rate: {}%".format(str(np.sum(df >= 1) + np.sum(df == 0)), str(np.sum(df >= 1) / (np.sum(df >= 1) + np.sum(df == 0)))))
